...
$(".next").click(function(){
    ...
    if(currentid=='next-1'){
        ...
        if(employment_type==""){
            $('#employment_type_error').html('The Type Of Occupation Field Required');
            return false;
        ...
        var email=$('#email').val();
        var ajax1=$.ajax({
            url: SiteUrl+"/ajax/email_validation",
            method:'POST',
            cache: false,
            dataType:"json",
            data:{"email":email},
        ...
        var ajax2=$.ajax({
            url: SiteUrl+"/ajax/phone_validation",
            method:'POST',
            cache: false,
            dataType:"json",
        ...
        $.ajax({
            ...
            coId.each(function(){
                $("#other_pancard_error"+$(this).val()).html("");
            });
            var iserror=true;
            ...
            other_pannumber.each(function(){
                if($(this).val()==""){
                    $(this).siblings(".other_pan_error").html("The Pan Number Field is Required");
                    iserror=false;
                    return false;
                }else{
        ...
        /* $.ajax({
            url: SiteUrl+"/ajax/send_loan_otp",
            method:'POST',
            dataType:"json",
            data:{"mobile_number":$('#phone').val()},
            success: function(result){
...
function VerifyOtp(){
    var otp=$('#loan_otp').val();
    $('#loan_otp_errorr').html('');
    if(otp==""){
        $('#loan_otp_errorr').html('Invalid Otp');
       return false;
    }else{
       var mobile_number=  $('#phone').val();
        $.ajax({
            url: SiteUrl+"/ajax/loan_verify_otp",
            method:'POST',
            dataType:"json",
...
function getsuccess(){
    ...
    $.ajax({
        type: "POST",
        url: SiteUrl+"/ajax/loan_registration",
        data: formData,
        processData: false,
        contentType: false,
...
function AddCoApplicant(){
    ...
    '<div class="form-row mt-4">'+
        '<div class="col-12 col-sm-6">'+
            '<div class="md-input">'+
                '<input class="md-form-control" type="text" title="Name of co-applicant" name="co_name[]" required=""/>'+
                '<label>Name</label>'+
                '<small class="text-danger invalid"></small>'+
            '</div>'+
        '</div>'+
        '<div class="col-12 col-sm-6 mt-4 mt-sm-0">'+
            '<div class="md-input">'+
                '<input class="md-form-control"  type="text" title="Relationship with co-applicant" name="co_relationship[]" required=""/>'+
    ...
    '<div class="form-row mt-4">'+
        '<div class="col-12 col-sm-6">'+
            '<div class="md-input">'+
                '<input class="md-form-control"  name="co_pan_number[]" type="text" title="PAN No. of co-applicant" required=""/>'+
                '<label>PAN</label>'+
...
function MobileValidate() {
    ...
    if (mob.test(txtMobile.value.trim()) == false) {
        return false;
    }
    return true;
}
...
$( "#personal-form-submit" ).click(function(){
    $('#first_name_error').html('');
        ...
        $.when(ajax1,ajax2).done(function(a1, a2){ 
            if(a1[0].status=="success" && a2[0].status=="success"){
                if($("#noti_6").prop('checked') == false){
                    $('#term_error').html('Please Agree T&C');
                    return false;
                }
                $.ajax({
                    url: SiteUrl+"/ajax/incomplete_form",
                    method:'POST',
                    cache: false,
                    dataType:"json",
                    data:{"mobile_number":phone,'email':email,
                        'employment_type':employment_type,
                        'first_name':first_name,'last_name':last_name,
                        'age':age,'das_agent_id':$('#das_agent_id').val(),
                        'other_app_user_id':$('#other_app_user_id').val(),
                        'date_of_birth':birthdate
                    },
                    success: function(result){
                        //console.log(result);
                        if(result.status=='fail'){
                            $('#registration_error').html('Registration Fail');
                        }else{
                            location.reload();
                        }
                    }
...
function DocumentUpload(){
    ...
    $.ajax({
        type: "POST",
        url: SiteUrl+"/ajax/loan_documents",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function (request, xhr) {
...
function UploadCoApplicantDocs(){
    ...
    $.ajax({
        type: "POST",
        url: SiteUrl+"/ajax/UploadCoApplicantDocs",
        data: formData,
        processData: false,
        contentType: false,
        beforeSend: function (request, xhr) {
...
function UploadBusinessData(){  
    ...
    $.ajax({
        type: "POST",
        url: SiteUrl+"/ajax/UploadBusinessData",
        data: formData,
        processData: false,
        contentType: false,
        success: function(response) {
...
function GetStatus(){
    ...
    $('#password_error').html('');
    if($('#caseid').val()==''){
        $('#caseid_error').html('The Case Id field is Required');
        return false;
    }
    if($('#password').val()==''){
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.status=='success'){
                ...
                '<h4>Personal info</h4>'+
                    '<ul>'+
                        '<li><span>Name:</span> '+response.data.full_name+'</li>'+
                        '<li><span>Mobile Number:</span> '+response.data.mobile_number+'</li>'+
                        '<li><span>File ID:</span> '+response.data.file_id+'</li>'+
                        '<li><span>Status:</span> '+response.data.status+'</li>'+
                    '</ul>'+
                '</div>';
                $('#addstatus').html(html);
                $('#status-footer').html('<button type="button" data-dismiss="modal" class="btn btn-default">CLOSE</button>');
            }else{
                if(response.data.caseid_error!=""){
                    $('#caseid_error').html(response.data.caseid_error);
                }
...
function OtherPincode(value){
    if(value=='Other'){
        ...
        '<div class="col-12 col-sm-6 mt-4 mt-sm-0">'+
            '<div class="md-input">'+
                `<input class="md-form-control" value="`+other_pincode+`" autocomplete="off" id="other_pincode" type="text" oninput="this.value = Math.abs(this.value);" minlength="6" maxlength="6" onkeyup="DataSave(this.id,this.value)" required="" title="Other Pincode" />`+
