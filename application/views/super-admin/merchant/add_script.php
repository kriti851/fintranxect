...
$(".next").click(function(){
    ...
    if(currentid=='next-1'){
        ...
        $.when(ajax1,ajax2).done(function(a1, a2){
            if(a1[0].status=="success" && a2[0].status=="success"){
                $.ajax({
                    ...
                    cache: false,
                    dataType:"json",
                    data:{"phone":phone,'email':email,'first_name':first_name,'last_name':last_name,'age':age,'employment_type':employment_type,'merchant_id':$('#merchant_id').val(),'dsaid':$('#dsaid').val()},
                    success: function(result){
        ...
        if($('#reference').val()==""){
            $('#reference_error').html('The Refrence Name Field Required');
            return false;
        }
        if($('#reference_number').val()==""){
            $('#refernece_number_error').html('The Refrence Number Field Required');
            return false;
        }else{
