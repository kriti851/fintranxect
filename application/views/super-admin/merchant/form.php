                        ...
                        <div class="col-sm-12">
                            <div class="col-sm-12 mt-4 text-left">
                                <form id="msform">
                                    ...
                                    <fieldset <?php if(!empty($active)){ ?>style="transform: scale(0.8); opacity: 0; display: none;" <?php } ?>>
                                        <h3 class="multisteps-form__title">Personal info</h3>
                                        <div class="form-row mt-4">
                                            <div class="col-12 col-sm-12">
                                                <label style="display:none" id="dsaid_label" class="accup">Partner</label>
                                                <select class="multisteps-form__input form-control" id="dsaid" title="Partner" >
                                                    <option value="">Partner</option>
                                                    <option value="1">Super Admin</option>
                                                    <?php foreach($dsalist as $dsa){  ?>
                                                        <option value="<?php echo $dsa->user_id; ?>" <?php if(!empty($users->created_by) && $users->created_by==$dsa->user_id){ echo "selected"; } ?>><?php echo $dsa->company_name; ?></option>
                                                    <?php }  ?>
                                                </select>
                                                <small class="text-danger" id="dsaid_error"></small>
                                            </div>
                                        </div>
                                        <div class="form-row mt-4">
                                            <div class="col-12 col-sm-6">
                                                <div class="md-input">
                                                    <input class="md-form-control" id="first_name" title="First Name" value="<?php if(!empty($full_name)){ echo $full_name[0];} ?>" type="text" required=""/>
                                        ...
                                        <div class="form-row mt-4">
                                            ...
                                            <div class="col-12 col-sm-6 mt-4 mt-sm-0">
                                                <div class="md-input">
                                                    <input class="md-form-control" title="Age" value="<?php if(!empty($users->age)){ echo $users->age; } ?>" id="age" type="number" oninput="this.value = Math.abs(this.value)" required=""/>
                                                    <label>Age</label>
...
<script>
    document.getElementById('dsaid').onchange=function(){
        DsaLabel();
    }
    function DsaLabel(){
        if(document.getElementById('dsaid').value==''){
            document.getElementById('dsaid_label').setAttribute('style','display:none');
            document.getElementById('dsaid').setAttribute('style','');
        }else{
            document.getElementById('dsaid_label').setAttribute('style','');
            document.getElementById('dsaid').setAttribute('style','padding-top:17px;');
        }
    }
    DsaLabel();
