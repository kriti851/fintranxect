                            ...
                            <div class="float-right">
								...
								<h5>
                                    <?php if(SubPermission(17)){ ?>
									    <a class="button viewbutton" href="<?php echo dsa_url('merchant/DownloadApplicant2/'.$record->user_id) ?>" class="ediprofile-button"><i class="fa fa-download"></i>Download</a>
                                    <?php } ?>
                                </h5>
								</div>
							</div>
							...
							<div class="body">
								<div class="content">
										...
										<div class="info">
											...
											<span class="company">
												<a href="#"><b>Status:</b> <?php $status =""; if(!empty($record->lender_status) && !empty($record->lender_status)){ echo $status= $record->lender_status; }elseif($record->status!=""){ echo $status=$record->status; }else{ echo $status= 'RECEIVED'; } ?></a>
											</span>
                            ...
                            <?php if(!empty($record->aadhar_number)){  ?>
                                <div style="width:100%" class="quote-imgs-thumbs">
                                    ...
                                    <?php if(!empty($record->aadhar_image)){ $imagearray=explode(',',$record->aadhar_image); foreach($imagearray as $file){ ?>
                                        ...
                                        <?php }elseif(pathinfo($file,PATHINFO_EXTENSION) == 'doc' || pathinfo($file,PATHINFO_EXTENSION) == 'docx'){ ?>
                                            <div class="m-2 img-preview-thumb"><a href="javascript:void(0)" onclick="ShowLargeDoc(`https://view.officeapps.live.com/op/embed.aspx?src=<?php echo urlencode(base_url('uploads/merchant/aadharcard/'.$file)); ?>`)" style="font-size:60px;"  class="text-primary"><i class="fa fa-file-word" aria-hidden="true"></i></a></div>
                                    <?php }else{ ?>
